...
NN_values = c(500, 800, 1000, 2000, 4000)

sum_all = lapply(NN_values,function(NN){
  cat("NN = ",NN,"\n")
  
  result_files = mixedsort(list.files(path = paste0("~/Desktop/results_new/resultsNN_",NN)))
  results_list = lapply(result_files, function(file) {
    load(paste0("~/Desktop/results_new/resultsNN_",NN,"/", file))
    list(
      truth = truth,
  ...
  truth = do.call(rbind,lapply(results_list, `[[`, "truth"))
  truth = truth[, c('mean', 'power2', 'scale', 'sill', 'smooth')] %>%t()
  # truth = truth[, c('mean', 'power2', 'scale', 'sill')] %>%t()
  ...
  length_CI_F = rowMeans(ci_upper_F - ci_lower_F, na.rm = T) # calculate length of confidence interval
  
  ci_lower_M = est_M - 1.96*se_M
  ci_upper_M = est_M + 1.96*se_M
  coverage_M = rowMeans(((ci_lower_M) <= truth) & (truth <= (ci_upper_M)),na.rm = T)
  length_CI_M = rowMeans(ci_upper_M - ci_lower_M, na.rm = T) # calculate length of confidence interval
  
  ci_lower_C = est_C - 1.96*se_C
  ci_upper_C = est_C + 1.96*se_C
  coverage_C = rowMeans(((ci_lower_C) <= truth) & (truth <= (ci_upper_C)),na.rm = T)
  length_CI_C = rowMeans(ci_upper_C - ci_lower_C, na.rm = T) # calculate length of confidence interval
  
  ci_lower_T = est_T - 1.96*se_T
  ci_upper_T = est_T + 1.96*se_T
  coverage_T = rowMeans(((ci_lower_T) <= truth) & (truth <= (ci_upper_T)),na.rm = T)
  length_CI_T = rowMeans(ci_upper_T - ci_lower_T, na.rm = T) # calculate length of confidence interval
  ...
  rst = list(
    ...
    lengthCI_F = length_CI_F,
    lengthCI_M = length_CI_M,
    lengthCI_C = length_CI_C,
    lengthCI_T = length_CI_T
  )
  return(rst)
})
...
library(ggplot2)
library(ggthemes)
results_df %>% pivot_longer(cols = starts_with(c('bias', 'se', 'mse', 
                                                 'coverage', 'lengthCI')),
                            names_to = c('metric', 'method'),
                            names_sep = '[_]',
                            values_to = "value") %>%
  filter(param %in% c('mean','sill', 'smooth')) %>%
  mutate(method = factor(method, levels = c('M', 'C', 'F', 'T')),
         metric = factor(metric, levels = c('bias', 'se', 'mse', 
                                            'coverage', 'lengthCI'))) %>%
